...
async function saveProduct(event) {
    ...
    console.log(prod)
    const response = await postMethodPayload('http://localhost:8080/api/product/admin/create-update', prod)
...
const AdminAddProduct = ()=>{
    ...
    useEffect(()=>{
        const getBlog= async() =>{
            ...
            var id = uls.searchParams.get("id");
            if(id != null){
                var response = await getMethod('http://localhost:8080/api/product/public/findById?id=' + id);
                var result = await response.json();
                setProduct(result)
        ...
        const getDanhMuc= async() =>{
            var response = await getMethod("http://localhost:8080/api/category/public/find-all");
                    ...
                    <div class="form-add">
                        <form class="row" onSubmit={saveProduct} method='post'>
                            <div class="col-md-4 col-sm-12 col-12">
                                ...
                                {/* <select name="unit" defaultValue={product==null?'':product.unit} class="form-control"/> */}
                                <select name="unit" class="form-control">
                                    <option value="Người" selected={product==null?true:product.unit=="Người"}>Người</option>
                                    <option value="Combo" selected={product==null?false:product.unit=="Combo"}>Combo</option>
                                    <option value="Bát" selected={product==null?false:product.unit=="Bát"}>Bát</option>
                                    <option value="Đĩa" selected={product==null?false:product.unit=="Đĩa"}>Đĩa</option>
                                </select>
                            ...
                            <div class="col-md-8 col-sm-12 col-12">
                                ...
                                <label class="lb-form lbmotadv">Mô tả món ăn</label>
